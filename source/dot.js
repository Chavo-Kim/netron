    ...
    constructor(metadata, cfg) {
        ...
        for (;;) {
            ...
            if(line.includes('->')) {
                ...
                if(section) {
                    ...
                    const id = line.substring(0, line.indexOf('[')-1);
                    section = {
                        id: id,
        ...
        const option_find_int = (options, key, defaultValue) => {
            ...
            if (value !== undefined) {
                ...
                if (!Number.isInteger(number)) {
                    throw new dot.Error("Invalid int option '" + JSON.stringify(options[key]) + "'.");
                }
                return number;
            }
            return defaultValue;
        ...
        const make_shape = (dimensions, source) => {
            if (dimensions.some((dimension) => dimension === 0 || dimension === undefined || isNaN(dimension))) {
                throw new dot.Error("Invalid tensor shape '" + JSON.stringify(dimensions) + "' in '" + source + "'.");
            }
            return new dot.TensorShape(dimensions);
        };

        const load_weights = (name, shape, visible) => {
            const data = weights ? weights.read(4 * shape.reduce((a, b) => a * b)) : null;
            const type = new dot.TensorType('float32', make_shape(shape, 'load_weights'));
            const initializer = new dot.Tensor(type, data);
            const argument = new dot.Argument('', null, initializer);
            return new dot.Parameter(name, visible === false ? false : true, [ argument ]);
        };
        ...
        const make_convolutional_layer = (layer, prefix, w, h, c, n, groups, size, stride_x, stride_y, padding, batch_normalize) => {
            ...
            layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'make_convolutional_layer'));
        };
        ...
        const make_connected_layer = (layer, prefix, inputs, outputs, batch_normalize) => {
            ...
            layer.outputs[0].type = new dot.TensorType('float32', make_shape([ outputs ], 'make_connected_layer'));
        };

        if (sections.length === 0) {
            throw new dot.Error('Config file has no sections.');
        }
        ...
        switch (net.type) {
            ...
            default: {
                throw new dot.Error("Unexpected '[" + net.type + "]' section. First section must be [net] or [network].");
            }
        }

        const inputType = params.w && params.h && params.c ?
            new dot.TensorType('float32', make_shape([ params.w, params.h, params.c ], 'params-if')) :
            new dot.TensorType('float32', make_shape([ params.inputs ], 'params-else'));
        const inputName = 'input';
        params.arguments = [ new dot.Argument(inputName, inputType, null) ];
        this._inputs.push(new dot.Parameter(inputName, true, params.arguments));
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            section.layer = {
                inputs: [],
                weights: [],
                outputs: [ new dot.Argument(section.name, null, null) ]
            };
        }
                ...
                switch (section.type) {
                    ...
                    case 'deconvolutional': {
                        ...
                        if (shape[0] !== params.w || shape[1] !== params.h || shape[2] !== params.c) {
                            throw new dot.Error('Layer before convolutional layer must output image.');
                        }
                    ...
                    case 'local': {
                        const shape = layer.inputs[0].type.shape.dimensions;
                        if (shape[0] !== params.w || shape[1] !== params.h || shape[2] !== params.c) {
                            throw new dot.Error('Layer before avgpool layer must output image.');
                        }
                        const n = option_find_int(options, 'filters' , 1);
                        const size = option_find_int(options, 'size', 1);
                        const stride = option_find_int(options, 'stride', 1);
                        ...
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'local'));
                        if (activation !== 'logistic' && activation !== 'none') {
                            section.chain.push({ type: activation });
                        }
                        break;
                    ...
                    case 'batchnorm': {
                        ...
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'batchnorm'));
                        break;
                    }
                    case 'activation': {
                        layer.out_h = params.h;
                        ...
                        layer.out_c = params.c;
                        layer.out = layer.in;
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'activation'));
                        break;
                    }
                    case 'max':
                    case 'maxpool': {
                        const shape = layer.inputs[0].type.shape.dimensions;
                        if (shape[0] !== params.w || shape[1] !== params.h || shape[2] !== params.c) {
                            throw new dot.Error('Layer before maxpool layer must output image.');
                        }
                        ...
                        if (antialiasing) {
                            ...
                            layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'maxpool'));
                        }
                        layer.out = layer.out_w * layer.out_h * layer.out_c;
                        break;
                    }
                    case 'avgpool': {
                        ...
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'avgpool'));
                        break;
                    }
                    case 'crnn': {
                        ...
                        const padding = pad ? (size >> 1) : option_find_int(options, 'padding', 0);
                        const batch_normalize = option_find_int(options, 'batch_normalize', 0);
                        layer.input_layer = { weights: [], outputs: [ new dot.Argument('', null, null) ] };
                        ...
                        layer.self_layer = { weights: [], outputs: [ new dot.Argument('', null, null) ] };
                    ...
                    case 'gru': {
                        ...
                        layer.out = outputs;
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ outputs ], 'gru'));
                        break;
                    }
                    case 'lstm': {
                        const inputs = params.inputs;
                        ...
                        layer.out_c = outputs;
                        layer.out = outputs;
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ outputs ], 'lstm'));
                        weights = null;
                        break;
                    }
                    case 'conv_lstm': {
                        ...
                        if (peephole) {
                            layer.vf = { weights: [], outputs: [ new dot.Argument('', null, null) ] };
                            ...
                            layer.vi = { weights: [], outputs: [ new dot.Argument('', null, null) ] };
                            ...
                            layer.vo = { weights: [], outputs: [ new dot.Argument('', null, null) ] };
                        ...
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'conv_lstm'));
                        break;
                    }
                    case 'softmax': {
                        layer.out_w = params.w;
                        ...
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out ], 'softmax'));
                        break;
                    }
                    case 'dropout': {
                        layer.out_w = params.w;
                        ...
                        layer.out_c = params.c;
                        layer.out = params.inputs;
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'dropout'));
                        break;
                    }
                    case 'upsample': {
                        ...
                        layer.out_c = params.c;
                        layer.out = layer.out_w * layer.out_h * layer.out_c;
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'upsample'));
                        break;
                    }
                    case 'crop': {
                        const shape = layer.inputs[0].type.shape.dimensions;
                        if (shape[0] !== params.w || shape[1] !== params.h || shape[2] !== params.c) {
                            throw new dot.Error('Layer before crop layer must output image.');
                        }
                        ...
                        layer.out_c = params.c;
                        layer.out = layer.out_w * layer.out_h * layer.out_c;
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'crop'));
                        break;
                    }
                    case 'yolo': {
                        ...
                        layer.out = layer.out_h * layer.out_w * layer.out_c;
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'yolo'));
                        break;
                    }
                    case 'Gaussian_yolo': {
                        const classes = option_find_int(options, 'classes', 20);
                        ...
                        layer.out = layer.out_h * layer.out_w * layer.out_c;
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'Gaussian_yolo'));
                        break;
                    }
                    case 'region': {
                        ...
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ params.h, params.w, num, (classes + coords + 1) ], 'region'));
                        break;
                    }
                    case 'cost': {
                        layer.out = params.inputs;
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out ], 'cost'));
                        break;
                    }
                    case 'reorg': {
                        const stride = option_find_int(options, 'stride', 1);
                        ...
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out ], 'reorg'));
                        break;
                    }
                    case 'route': {
                        ...
                        if (layers.length > 0) {
                            ...
                            if (infer) {
                                layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'route'));
                            }
                        }
                        else {
                            infer = false;
                    ...
                    case 'scale_channels': {
                        ...
                        if (from) {
                            ...
                            layer.out = layer.out_w * layer.out_h * layer.out_c;
                            layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out_w, layer.out_h, layer.out_c ], 'shortcut|scale_channels|sam'));
                        }
                    ...
                    case 'shortcut': {
                        ...
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ params.w, params.h, params.c ], 'shortcut|scale_channels|sam'));
                        if (activation !== 'linear' && activation !== 'none') {
                            section.chain.push({ type: activation });
                        }
                        break;
                    ...
                    case 'detection': {
                        ...
                        layer.out_c = params.c;
                        layer.out = params.inputs;
                        layer.outputs[0].type = new dot.TensorType('float32', make_shape([ layer.out ], 'detection'));
                        break;
                    }
                    default: {
                        infer = false;
        ...
        for (let i = 0; i < sections.length; i++) {
            this._nodes.push(new dot.Node(metadata, net, sections[i]));
        }
